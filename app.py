...
def add_like(message_id):
    """Add a like for the currently-logged-in user to a message."""
    
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")
    
    liked_message = Message.query.join(Likes).filter(Message.id == message_id).first()
...
def homepage():
    ...
    if g.user:
        ...
        # if there are >100 messages from followed users, only display 100 messages
        # if there are <100 messages from followed users, display all of those messages first,
        # then display the remaining messages from non-followed users up to 100.
        ...
        if filtered_count < 100:
            non_filtered_messages = (Message
                                    .query
                                    .join(Message.user)
                                    .filter(~User.id.in_(following_ids))
                                    .order_by(Message.timestamp.desc())
                                    .limit(100 - filtered_count)
                                    .all())

            messages = filtered_messages + non_filtered_messages
        else:
